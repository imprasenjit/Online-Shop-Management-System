    ...
    $.fn.pekeUpload = function (options) {
        ...
        var pekeUpload = {
            ...
            replacehtml: function () {
                ...
                switch (options.dragMode) {
                    ...
                    case false:
                        switch (options.bootstrap) {
                            case true:
                                html = '<a href="javascript:void(0)" class="btn btn-sm btn-primary btn-upload pkuparea"> <i class="glyphicon glyphicon-upload"></i> ' + options.btnText + "</a>";
                                break;
                            case false:
                ...
                this.container = $('<div class="pekecontainer"></div>').insertAfter(this.uparea);
            },
            selectfiles: function () {
                this.obj.click();
            },
            handlebuttonevents: function () {
                this.obj.bind("change", this.obj.selector, function () {
            ...
            addRow: function (file) {
                ...
                switch (options.bootstrap) {
                    case true:                        
                        var newRow = $('<div class="row pkrw" style="width:95%" rel="' + i + '" id="row_'+options.data.file+'_' + i + '" ></div>').appendTo(this.container);
                        if (options.showPreview) {
                            var prev = $('').appendTo(newRow);
                            this.previewFile(prev, file);
                        }
                        //var finfo = $('<div class="col-md-10"></div>').appendTo(newRow);
                        if (options.showFilename) {                            
                            var progress = $('<div class="col-md-7"><div class="progress" style="height:35px;"><div class="pkuppbr progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="line-height:30px"></div></div></div>').appendTo(newRow);
                            newRow.append('<div class="col-md-3 pekefilename" style="overflow:hidden;padding-left:0px;padding-right:0px;text-overflow:ellipsis;font-size:'+options.fontSize+'px;">' + file.name + "</div>");;
                            if (options.showPercent) {
                                progress.find("div.progress-bar").text("0%");
                            }
                        }
                        var dismiss = $('<div class="col-md-2"></div>').appendTo(newRow);
            ...
            upload: function (file, pos) {
                ...
                var pkcontainer=this.container;
                $.ajax({
                    url: options.url,
                    type: "POST",
                    data: formData,
                    ...
                    success: function (data) {
                        if (data == 1 || data.success == 1) {
                            ...
                            $('div.row[rel="' + pos + '"]').find(".pkuppbr").css("width", "100%").addClass('progress-bar-success').empty().append('Successfully Uploaded');
                            $('#row_'+options.data.file+'_'+pos+'').append('<input type="hidden" name="'+data.name+'[]" class="uplodedfile" value="'+data.upload_data.file_name+'">');

...
class Purchase_orders extends Aipl_admin
{
    ...
    public function search()
    {
        ...
        if ($this->uri->segment(3)=='search') {
            $config['base_url'] = base_url() . 'purchase_orders/search/' . $keyword;
        } else {
            $config['base_url'] = base_url() . 'purchase_orders/index/';
        }
        $config['total_rows'] = $this->purchase_order_model->search_total_rows($keyword);
        $config['per_page'] = 10;
        $config['uri_segment'] = 5;
       // $config['suffix'] = '.html';
        $config['first_url'] = base_url() . 'purchase_orders/search/'.$keyword.'.html';
        $this->pagination->initialize($config);
        $start = $this->uri->segment(5, 0);
        $purchase_orders = $this->purchase_order_model->search_index_limit($config['per_page'], $start, $keyword);
        $data = array(
            'purchase_orders_data' => $purchase_orders,
            'keyword' => $keyword,
        ...
        $this->load->view('admin/purchase_orders/purchase_orders_list', $data);
        $this->load->view('admin/requires/footer');
    }
    ...
    {
        $row = $this->purchase_order_model->get_by_id($id);
        if ($row) {
            $data = array(
			'id' => $row->id,
		...
		$this->load->view('admin/requires/header',array('title'=>'purchase_orders'));
		$this->load->view('admin/purchase_orders/purchase_orders_read', $data);
		$this->load->view('admin/requires/footer');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('admin/purchase_orders'));
        }
    }
